...
public partial class Account_seller : ContentPage
{
    ...
    private async void OnEmergencyContactClicked(object sender, EventArgs e)
    {
        try
        {
            await Navigation.PushAsync(new SubdiHub_v1.Components.Dashboards.Admin.EmergencyContact());
        }
        catch (Exception ex)
        {
    ...
    {
        await Shell.Current.GoToAsync("Edit_profile_S");
